...
export function ProductHospital() {
  ...
  return (
    <div >
      ...
      {hospital.map((pro) => <CardCollection product={pro} id={pro.id} deleteButton={<Tooltip><IconButton><DeleteIcon className="but1" fontSize="small"   onClick={()=>deletePet(pro.id)}/></IconButton></Tooltip>} />)}
      </div>
    </div>
  );
}
function CardCollection({ product,id,deleteButton }) {
  ...
  return (
   ...
   <FavoriteBorderIcon/>
      ...
      {deleteButton}
        <CardActions>
        <Button size="small" onClick={()=>navigate(`/hospital/${id}`)}>Detiale</Button>
           </CardActions>

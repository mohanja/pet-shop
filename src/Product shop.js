 ...
 const getPets=()=>{
  ...
  return (
    <div >
      ...
      {shop.map((pro) => <CardCollection   product={pro} id={pro.id} deleteButton={<Tooltip><IconButton><DeleteIcon className="but1" fontSize="small"   onClick={()=>deletePet(pro.id)}/></IconButton></Tooltip>} />)}
      </div>
    </div>
  );
}
function CardCollection({ product,id,deleteButton }) {
  ...
  return (
   ...
   <FavoriteBorderIcon/>
        ...
        {deleteButton}
        <CardActions>
        <Button size="small" onClick={()=>navigate(`/shop/${id}`)}>Detiale</Button>
           </CardActions>
